...
workflow_ORDER = [
    "s1_ligand_filter", "s2_ml_affinity", "s3_docking", "s4_md_refinement", "s5_fep_ranking",
...
def plot_gantt(results: dict, out_dir: Path) -> None:
    ...
    for ax, cfg in zip(axes, cfgs):
        ...
        ax.set_title(f"{_cname(cfg)}  (avg wall={t_end:.1f}s)", fontsize=9,
                     color=CFG_COLORS.get(cfg, "black"))
        ax.grid(axis="x", linestyle="--", alpha=0.35)

    plt.suptitle("Stage execution overlap per ADR policy\n"
...
def plot_cascade_funnel(results: dict, out_dir: Path) -> None:
    ...
    ax.set_title("Total compute launched per ADR policy\n"
                 "(stacked by stage; lower = less wasted work)", fontsize=15)
...
def plot_time_to_target(
    ...
    for cfg in cfgs:
        ...
        if len(rep_ts) >= target_n:
            ...
            final_n = len(rep_ts)
            ax.text(final_t + 0.3, final_n + 0.1, f"reached {final_n}",
