...
async def _run_once(config: dict, policy_kind: str, seed_offset: int) -> dict:
    ...
    from run_campaign import _build_registry, _build_adr_operator
    ...
    operator, tick = _build_adr_operator(cm, asyncflow, adr_cfg,
                                         policy_override=policy_kind)
...
async def run_benchmark(config_path: str, n_runs: int, out_path: str,
    ...
    for policy in policies:
        ...
        for run_idx in range(n_runs):
            ...
            try:
                ...
                elapsed = time.time() - t0
                print(f"FAILED: {exc}")
                policy_results.append({"error": str(exc), "wall_time_s": elapsed,
                                       "policy": policy})
    ...
    print(f"{'policy':8s}  {'best_score':>10s}  {'refine_done':>11s}  "
    ...
    for policy in policies:
        ...
        print(f"{policy:8s}  {avg_best:>10.4f}  {avg_ref:>11.1f}  "
              f"{avg_eval:>6.0f}  {avg_wall:>8.2f}s{dnf_tag}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Dummy minimization — ADR policy benchmark"
    )
    parser.add_argument("--config", default="config.yaml",
                        help="Campaign config YAML (default: config.yaml)")
    parser.add_argument("--runs", type=int, default=5,
                        help="Runs per policy (default: 5)")
