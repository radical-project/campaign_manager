...
def plot_wall_time(results: dict, out_dir: Path) -> None:
    ...
    medians = [_median([r["wall_time_s"] for r in results[c] if r.get("wall_time_s")])
               for c in cfgs]
...
def plot_time_to_miniapps(results: dict, out_dir: Path) -> None:
    ...
    cfgs = [c for c in results if c not in _EXCLUDE]
    medians = [
        _median([r.get("time_to_first_miniapps_s") for r in results[c]
                 if r.get("time_to_first_miniapps_s") is not None])
        for c in cfgs
    ]
    base_runs = results.get(BASELINE_KEY, [])
    baseline  = _median([r.get("time_to_first_miniapps_s") for r in base_runs
                         if r.get("time_to_first_miniapps_s") is not None])
    have_base = baseline is not None and baseline > 0
    ...
    x    = np.arange(len(cfgs))
    bars = ax.bar(x, [_z(m) for m in medians],
                  color=[GROUP_COLORS["miniapps"]] * len(cfgs), alpha=0.85)
    # policy-colored outline
    for bar, cfg in zip(bars, cfgs):
        bar.set_edgecolor(CFG_COLORS.get(cfg, "#888"))
        bar.set_linewidth(2.5)
    for i, cfg in enumerate(cfgs):
        vals = [r.get("time_to_first_miniapps_s") for r in results[cfg]
                if r.get("time_to_first_miniapps_s") is not None]
...
def plot_pipeline_gantt(results: dict, out_dir: Path) -> None:
    ...
    for pi, cfg in enumerate(reversed(cfgs)):
        ...
        for gi, group in enumerate(GROUP_ORDER):
            ...
            t0s = _valid(t0s); t1s = _valid(t1s)
            if not t0s or not t1s:
                continue
            t0 = statistics.mean(t0s)
            t1 = statistics.mean(t1s)
            ax.barh(y, t1 - t0, left=t0, height=h * 0.85,
                    color=GROUP_COLORS[group], alpha=0.88)
            mid = t0 + (t1 - t0) / 2
            ax.text(mid, y, GROUP_LABELS[group],
    ...
    ax.set_title("Pipeline Execution Windows per Policy\n"
...
def plot_throughput(results: dict, out_dir: Path) -> None:
    ...
    for i, cfg in enumerate(cfgs):
        runs = [r for r in results[cfg] if "group_stats" in r]
        vals = [
            _mean([r["group_stats"].get(g, {}).get("n_finished") for r in runs])
            for g in groups
        ]
        offset = (i - len(cfgs) / 2 + 0.5) * width
        bars = ax.bar(x + offset, [_z(v) for v in vals], width * 0.9,
                      label=_cname(cfg), color=CFG_COLORS.get(cfg, "#888"), alpha=0.85)
