...
class RecallTracker:
    ...
    def observe(self, predicted: float, actual: float) -> None:
        ...
        if recall < self.floor:
            ...
            if self._consecutive_low >= self.breaches_to_escalate and not self._drift_active:
    ...
    def recall_at_k(self, k: int) -> float:
        ...
        if len(self._pairs) < k:
            return 1.0
        preds = sorted(range(len(self._pairs)), key=lambda i: -self._pairs[i][0])[:k]
        actuals = sorted(range(len(self._pairs)), key=lambda i: -self._pairs[i][1])[:k]
...
def build_default_surrogate(
    ...
    if enable_recall:
        tracker = RecallTracker(
            ...
            on_recall_drift=on_recall_drift,
        )
