    ...
    *objective:* The campaigns objective
    ...
    def __init__(self, campaign, resources, objective=None, planner='random'):
        ...
        for workflow in self._campaign:
            self._execution_status[workflow['id']] = st.NEW
    ...
    def work(self):
        ...
        if self._plan is None:
            self._plan = self._planner.plan(campaign=self._campaign,
                                      reources=self._resources,
                                      num_oper=[something])
        ...
        if self._verify_objective():
            ...
            resource = list()  # The selected resources
            for (workflow, resource, start_time, _) in self._plan:
                if start_time == self._time:
            ...
            raise RuntimeError("Objective cannot be satisfied. Ending execution")
    ...
    def monitor(self):
        ...
        while self._workflows_to_monitor:
            ...
            if self._execution_status[workflow['id']]['state'] not in st.FINAL:
